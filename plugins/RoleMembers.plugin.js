...
module.exports = (() => {
    ...
    const config = {
        info: {
            ...
            github:
                "https://github.com/rauenzi/BetterDiscordAddons/tree/master/Plugins/RoleMembers",
            github_raw:
        ...
        changelog: [
            {
                ...
                items: [
                    ...
                    "Popout is whole instead of transparent.",
                ],
            },
        ],
        main: "index.js",
    ...
    return !global.ZeresPluginLibrary
        ...
        : /**
       * It creates a popout window that displays a list of users in a role.
       ...
       */
        (([Plugin, Api]) => {
            const plugin = (Plugin, Api) => {
                ...
                const animate = DOMTools.animate
                    ? DOMTools.animate.bind(DOMTools)
                    : ({ timing = (_) => _, update, duration }) => {
                ...
                /* Creating a popout window for the role members. */
...
<div class="animatorBottom-L63-7D translate-PeW1wK didRender-2SiRlm popout-role-members" style="margin-top: 0;">
    <div class="container-2O1UgZ role-members-popout">
        ...
        <div>
            <div class="list-3cyRKU none-2-_0dP scrollerBase-_bVAAt role-members" dir="ltr" style="overflow: hidden scroll; padding-right: 0px; max-height: 400px;">
                ...
                /* Creating a template for the user list. */
    ...
    <div class="itemLabel-27pirQ">
        <span class="username">{{username}}</span><span class="discriminator-2jnrqC">{{discriminator}}</span>
                ...
                /* Patching the role mention and the guild context menu. */
                return class RoleMembers extends Plugin {
                    ...
                    /**
                     * It removes the popout window that shows the roles of a user
                     */
                    onStop() {
                    ...
                    /**
                     * It adds an onClick event to the role mention, which then shows the role popout
                     */
                    patchRoleMention() {
                        const Pill = WebpackModules.getModule(
                            (m) => m?.default.displayName === "RoleMention"
                        );
                        Patcher.after(Pill, "default", (_, [props], component) => {
                            if (
                                ...
                                return;
                            if (
                                !component.props.className.toLowerCase().includes("mention")
                            )
                                return;
                    ...
                    /**
                     * It adds a submenu to the guild context menu that shows all the roles in the guild and
                     * when you click on one of them, it shows a popout with all the members of that role
                     * @param promiseState - This is a promise state object that is passed to the function. It
                     * has a property called cancelled which is a boolean. If the promise state is cancelled,
                     * then the function will not be called.
                     * @returns The return value is the menu item that is being created.
                     */
                    async patchGuildContextMenu(promiseState) {
                        const GuildContextMenu = await DCM.getDiscordMenu(
                            "GuildContextMenu"
                        );
                                ...
                                const item = DCM.buildMenuItem({
                                    ...
                                    action: (e) => {
                                        if (e.ctrlKey) {
                                            ...
                                            this.showRolePopout(
                                                e.target.closest(DiscordSelectors.ContextMenu.item),
                                                guildId,
                                                role.id
                                            );
                                        }
                    ...
                    /**
                     * It shows a popout with a list of members in a role
                     * @param target - The element that the popout will be attached to.
                     * @param guildId - The ID of the guild the role is in.
                     * @param roleId - The role id of the role you want to show the members of.
                     */
                    showRolePopout(target, guildId, roleId) {
                        ...
                        const popout = DOMTools.createElement(
                            Utilities.formatString(popoutHTML, {
                        ...
                        /* Creating a popout for each member of the role. */
                        const scroller = popout.querySelector(".role-members");
                        for (const member of members) {
                            const user = UserStore.getUser(member.userId);
                            const elem = DOMTools.createElement(
                                Utilities.formatString(itemHTML, {
                                    username: user.username,
                                    discriminator: "#" + user.discriminator,
                                    avatar_url: ImageResolver.getUserAvatarURL(user),
                                })
                            );
                            elem.addEventListener("click", () => {
                                setTimeout(
                                    () =>
                                        Popouts.showUserPopout(elem, user, { guild: guildId }),
                                    1
                                );
                            });
                    ...
                    /**
                     * It shows a popout menu when you click on a role in the roles list
                     * @param popout - The popout element to show
                     * @param relativeTarget - The element that the popout will be relative to.
                     */
                    showPopout(popout, relativeTarget) {
                        ...
                        document
                            .querySelector(
                                `#app-mount > ${DiscordSelectors.TooltipLayers.layerContainer}`
                            )
                            .append(popout);
                            ...
                            animate({
                                ...
                                update: function (progress) {
                                    ...
                                    if (endPoint > original)
                                        value = original + progress * (endPoint - original);
                        ...
                        if (offset.top + popout.offsetHeight >= maxHeight)
                            popout.style.top =
                                Math.round(maxHeight - popout.offsetHeight) + "px";
                        else popout.style.top = offset.top + "px";

                        this.listener = (e) => {
                            const target = e.target;
                            if (
                                !target.classList.contains("popout-role-members") &&
                                !target.closest(".popout-role-members")
                            ) {
                                popout.remove();
